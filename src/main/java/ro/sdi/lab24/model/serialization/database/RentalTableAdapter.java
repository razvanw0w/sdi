...
public class RentalTableAdapter implements TableAdapter<Rental.RentalID, Rental>
{
    ...
    @Override
    public Optional<Rental> read(Rental.RentalID rentalID, Connection connection) throws SQLException {
        ...
        ResultSet resultSet = selectStatement.executeQuery();
        Optional<Rental> rental = Optional.of(resultSet.next())
                .filter(boolValue -> boolValue)
                .map(boolValue -> {
                    try {
                        return new Rental(resultSet.getInt("movieid"),
